<script lang="ts">
    ...
    const allTimes = Array.from(days.values()).flatMap((day) =>
        day.map((command) => command.activationTime)
    ).sort();
...
<div data-componentName="schedule" class={css({
    ...
    {#each dayNames as dayName}
        <DayCommands
            name={dayName}
            commands={days.get(dayName) || []}
            timeSlots={activationTimes} />
    {/each}
</div>